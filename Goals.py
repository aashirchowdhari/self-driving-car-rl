...
# Class to represent a goal line in the racing game
class Goal:
    # Initialize goal with start and end points (x1, y1) and (x2, y2)
    def __init__(self, x1, y1, x2, y2):
        self.x1 = x1  # x-coordinate of start point
        self.y1 = y1  # y-coordinate of start point
        self.x2 = x2  # x-coordinate of end point
        self.y2 = y2  # y-coordinate of end point
        self.isactiv = False  # Flag to indicate if goal is active

    # Method to draw the goal line on the screen
    def draw(self, win):
        # Draw goal line in green
        pygame.draw.line(win, (0,255,0), (self.x1, self.y1), (self.x2, self.y2), 2)
        # If goal is active, redraw in red to highlight
        if self.isactiv:
            pygame.draw.line(win, (255,0,0), (self.x1, self.y1), (self.x2, self.y2), 2)

# Function to create and return a list of goal lines
def getGoals():
    # Initialize empty list to store goals
    goals = []
    # Create goal lines with specific coordinates
    ...
    # Add goals to the list (note: order differs from creation)
    goals.append(goal1)
    goals.append(goal2)
